    ...
    if (feature.properties && feature.properties.popupContent) {
      layer.bindPopup(feature.properties.popupContent).on("popupopen", () => {
        ...
        Connections.forEach((connection) => {
          ...
          let acdcIcon = "&#9107";
          try {
            if (CurrentType.Title.includes("AC")) acdcIcon = "&#9190;";
          } catch (e) {
            console.log(e);
          }
          info.innerHTML += `
...
<div class="buttons">
  ...
  const init = async () => {
    ...
    const bounds = JSON.stringify(map.getBounds()).replace(
      new RegExp('"', "g"),
      ""
    );
    const query = {
    ...
    // If connected to network, load data from db and save to indexedDB
    // If no connection, load data from indexedDB
    if (navigator.onLine) {
      try {
        data = await fetchGraphql(query);
        stations = data.stations;
        clearData("stationList");
        saveData("stationList", stations);
      } catch (error) {
        console.log(error.message);
      }
    } else {
      try {
        data = await loadData("stationList");
        stations = data[0];
      } catch (error) {
        console.log(error.message);
      }
  ...
  const addConnToList = (options, element) => {
    ...
    element.innerHTML += `<article id="conn-${connections.length - 1}">
    <a href="#" onclick="removeConnection(${
      connections.length - 1
    }); return false;"><i class="fas fa-trash-alt fa-2x"></i></a>
    ...
    <p><strong>Current type:</strong> ${
      curT.options[curT.selectedIndex].text
    }</p>
  ...
  addConnection.addEventListener("click", () => {
    const connectionOptions = [conTypes, levTypes, curTypes, quantity];
  ...
  connForm.addEventListener("submit", async (evt) => {
    ...
    const query = {
      query: `mutation {
  addStation(
    Connections: ${JSON.stringify(filteredConnections).replace(
      /\"([^(\")"]+)\":/g,
      "$1:"
    )},
...
`,
  ...
  const modifyConnection = (id) => {
    ...
    for (const option of modifyForm.elements["modify-connection-type"]
      .options) {
      if (option.value === conn.ConnectionType.id) option.selected = true;
    }
    for (const option of modifyForm.elements["modify-current-type"].options) {
      if (option.value === conn.CurrentType.id) option.selected = true;
    }
    for (const option of modifyForm.elements["modify-level"].options) {
      if (option.value === conn.LevelType.id) option.selected = true;
    }
  ...
  const saveModConnection = () => {
    ...
    newConn.ConnectionType.id =
      modifyForm.elements["modify-connection-type"].value;
    newConn.ConnectionType.Title =
      modifyForm.elements["modify-connection-type"].options[
        modifyForm.elements["modify-connection-type"].selectedIndex
      ].innerHTML;

    newConn.CurrentType.id = modifyForm.elements["modify-current-type"].value;
    newConn.CurrentType.Title =
      modifyForm.elements["modify-current-type"].options[
        modifyForm.elements["modify-current-type"].selectedIndex
      ].innerHTML;

    newConn.LevelType.id = modifyForm.elements["modify-level"].value;
    newConn.LevelType.Title =
      modifyForm.elements["modify-level"].options[
        modifyForm.elements["modify-level"].selectedIndex
      ].innerHTML;
